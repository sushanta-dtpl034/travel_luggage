...
class Assetmanagement extends CI_Controller
{
	...
	public function getoneasset()
	{
		...
		$assettransferlist = $this->Assetmodel->getAssettransferdetails($id);
		$response = array(
			...
			"AssetTransferDetails" => json_encode($assettransferlist),
		);
	...
	function single_qrcode_pdf_generate($noof_copy, $qrcodes_data){
		...
		$mpdf->AddPage();
		$inner_html = '';
		$qrcodefile_url = base_url('upload/qr-code/' . $qrcodes_data->UniqueRefNumber) . '.png"';
		if (!file_exists($qrcodefile_url)) {
			$this->load->library('myLibrary');
			$this->mylibrary->generate($qrcodes_data->UniqueRefNumber);
		}
				...
				<tr>
					<td>
						<table>
							<tr>
								<td width="30%">
									<img src="' . base_url('upload/qr-code/' . $qrcodes_data->UniqueRefNumber) . '.png" style="height:auto; width:100%;">
								</td>
								<td width="70%">
									<p style="font-size:22px;letter-spacing:3px"><b>' . $qrcodes_data->UniqueRefNumber . '</b></p>
									<table width="90%">
										<tr >
				...
				if ($i % 2 === 0) {
					if ($i == 0 && count($qrcodes_data) == 1) {
						...
						<table>
							<tr>
								<td width="30%">
									<img src="' . base_url('upload/qr-code/' . $qrcodes_data[$i]->UniqueRefNumber) . '.png" style="height:auto; width:100%;">
								</td>
								<td width="70%">
									<p style="font-size:22px;letter-spacing:3px"><b>' . $qrcodes_data[$i]->UniqueRefNumber . '</b></p>
					...
					<td>
						<table style="width:100%; display:none;">
							<tr style="display:none;">
								<td width="30%"></td>
								<td width="70%">
									<p style="font-size:22px; color:white"><b>' . $qrcodes_data[$i]->UniqueRefNumber . '</b></p>
									<p style="font-size:55px;color:white">.</p>
								</td>
							</tr>
					...
					';
						...
						<table>
							<tr>
								...
								<td width="70%">
									<p style="font-size:27px;letter-spacing:3px"><b>' . $qrcodes_data[$i]->UniqueRefNumber . '</b></p>
									<table style="width:90%">
										<tr>
											<td style="font-size:18px;letter-spacing:2px"><strong>Name</strong> </td>
											<td style="font-size:18px;letter-spacing:2px"><strong>:</strong></td>
					...
					<tr>
						<td>
							<table>
								<tr>
									<td width="30%">
										<img src="' . base_url('upload/qr-code/' . $data->UniqueRefNumber) . '.png" style="height:auto; width:100%;">
									</td>
									<td width="70%">
										<p style="font-size:22px;letter-spacing:3px"><b>' . $data->UniqueRefNumber . '</b></p>
										<table width="90%">
											<tr >
												<td style="font-size:18px;letter-spacing:2px"><strong>Name </strong></td>
												<td style="font-size:18px;letter-spacing:2px"><strong>:</strong></td>
	...
	public function exportAssetdetails(){
		...
		$sheet->setCellValue('B1', 'Unique Ref No');
		$sheet->setCellValue('C1', 'Asset Owner Company');
		$sheet->setCellValue('D1', 'Title');
		$sheet->setCellValue('E1', 'Location');
		$sheet->setCellValue('f1', 'Asset Current Location');
		$sheet->setCellValue('G1', 'Asset Category');
		$sheet->setCellValue('H1', 'Asset Sub Category');
		$sheet->setCellValue('I1', 'UIN');
		$sheet->setCellValue('J1', 'Puchase Date');
		$sheet->setCellValue('K1', 'Currency');
		$sheet->setCellValue('L1', 'Purchase Price');
		$sheet->setCellValue('M1', 'Status');
		$sheet->setCellValue('N1', 'Next Verification');
		$sheet->setCellValue('O1', 'Supervisor');
		$sheet->setCellValue('P1', 'User');
		$sheet->setCellValue('Q1', 'Auditor');
		$sheet->setCellValue('R1', 'CREATED BY');
		$sheet->setCellValue('S1', 'Attachment');
		...
		$rows = 2;
		$sn = 1;
		foreach ($result as $re_data) {
			$sheet->setCellValue('A' . $rows, $sn);
			$sheet->setCellValue('B' . $rows, $re_data['UniqueRefNumber']);
			$sheet->setCellValue('C' . $rows, $re_data['CompanyName']);
			$sheet->setCellValue('D' . $rows, $re_data['AssetTitle']);
			$sheet->setCellValue('E' . $rows, $re_data['Location']);
			$sheet->setCellValue('F' . $rows, $re_data['CurrentLocation']);
			$sheet->setCellValue('G' . $rows, $re_data['AsseCatName']);
			$sheet->setCellValue('H' . $rows, $re_data['AssetSubcatName']);
			$sheet->setCellValue('I' . $rows, $re_data['UIN']);
			$sheet->setCellValue('J' . $rows, $re_data['date']);
			$sheet->setCellValue('K' . $rows, $re_data['CurrencyCode']);
			$sheet->setCellValue('L' . $rows, $re_data['PurchasePrice']);
			$status = "";
			if ($re_data['isVerify'] == 1 && $re_data['isRemove'] != 1) {
				$status = "Verified";
			} elseif ($re_data['isRemove'] == 1 && $re_data['isVerify'] == 1) {
				$status = "Removed";
			} elseif ($re_data['isVerify'] != 1) {
				$status = "Pending";
			}
			$sheet->setCellValue('M' . $rows, $status);
			$sheet->setCellValue('N' . $rows, $re_data['VerificationDate']);

			$sheet->setCellValue('O' . $rows, $re_data['Supervisor']);
			$sheet->setCellValue('P' . $rows, $re_data['User']);
			$sheet->setCellValue('Q' . $rows, $re_data['Auditor']);
			$sheet->setCellValue('R' . $rows, $re_data['CreatedBy']);
			$asset_id = $re_data['AutoID'];
			$respictures = $this->Assetmodel->getpictures($asset_id);
			$image_name = "";
			$url_name = "";

			// $fn = 1;
			foreach ($respictures as $re_pic) {
				// if($image_name != ''){
				// 	$url_name .= '\n';
				// 	$image_name .= '\n';
				// }

				$image_url = base_url() . "upload/asset/" . $re_pic['ImageName'];

				//   $image_name = $re_pic['ImageName'];
				// $fn++;
			...
			//set wrap for newlines in cells
			//$spreadsheet->getActiveSheet()->getCell('A1')->setValue("hello\nworld");

			// $sheet->getCell('S'.$rows)->setValue($image_name);

			$sheet->setCellValue('S' . $rows, "View");
			$spreadsheet->getActiveSheet()->getStyle('S' . $rows)->getFont()->setBold(true);
			$spreadsheet->getActiveSheet()->getStyle('S' . $rows)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('1b55e3');

			// echo "<pre>";
			// echo $image_name;

			$spreadsheet->getActiveSheet()->getCell('S' . $rows)->getHyperlink()->setUrl("$image_url");


			// $spreadsheet->getActiveSheet()->getCell('S'.$rows)->setValue($url_name);
			// $spreadsheet->getActiveSheet()->getStyle('S'.$rows)->getAlignment()->setWrapText(true);
		...
		// $col = 19;
		// $spreadsheet->getActiveSheet()->getStyle($col.$rows)->getAlignment()->setWrapText(true);
		$writer = new Xlsx($spreadsheet);
		$writer->save("php://output");
	}
	...
	public function allocateasset_list(){
		...
		$data['incharge'] = $this->Usermodel->get_incharge($parentid);
		$data['supervisor'] = $this->Usermodel->get_supervisor($parentid);
		$this->load->view('superadmin/allocateasset_list', $data);
		$this->load->view('include/admin-footer');
	}
	...
	public function allocate_save()
	{
		...
		$asset_user = $this->input->post('allocate_incharge');
		$allocate_supervisor = $this->input->post('allocate_supervisor');
		$updateid = $this->input->post('updateid');
		$data = array(
			'Incharge' => $asset_user,
			'Supervisor' => $allocate_supervisor,
		);
		$where = array(
			'AutoID' => $updateid,
		);
	...
	public function assetlist()
	{
		...
		$result = $this->Assetmodel->assetlist($parentid);
		$status = 200;
		$this->output
	...
	{
		$Companylist = $this->Companymodel->CompanyList();
		$status = 200;
		$this->output
			->set_status_header($status)
			->set_content_type('application/json', 'utf-8')
	...
	public function Userlistdirectory()
	{
		$userlist = $this->wlguserlist->updateUsersData();
		foreach ($userlist['data'] as $result) {
			$EmployeeCode = $result['EmployeeCode'];
			$empresult = $this->Usermodel->getuser_byEmployeeCode($EmployeeCode);
			$parent_id = $this->session->userdata('userid');
			if ($this->session->userdata('GroupID') != '1') {
				$parent_id = $this->session->userdata('parentid');
			}
			$name = trim($result['FullName']);
			$email = trim($result['UserEmail']);
			$mobile = trim($result['UserMobile']);
			$Suffix = trim($result['Suffix']);
			$CountryCode = trim($result['CountryCode']);
	...
	//remove asset
	public function removeassetform_list()
	{
		$data['page_title'] = 'Asset List';
		$data['page_name'] = "List of Removed Asset ";
		$this->load->view('include/admin-header', $data);
		$this->load->view('include/sidebar');
		$this->load->view('include/topbar');
		$parentid = $this->session->userdata('parentid');
		...
		$data['warranty'] = $this->Assetmodel->getwarantytype($parentid);
		$data['clocation'] = $this->Assetmodel->getlocation($parentid);
		$this->load->view('superadmin/removedassetlist', $data);
		$this->load->view('include/admin-footer');
	}


	public function getremovedasset_list()
	{
		...
		$result = $this->Assetmodel->getremoveassetlist($parentid);
		$json_data['data'] = $result;
		// $json_data['role'] = null;
