  ...
  public function assetcat_list()
	...
	function cat_alreadyexist($key)
	{
		...
		public function myassets_list()
		{
			...
			$data['page_name'] = "List of My Assets";
			$this->load->view('include/admin-header',$data);
			$this->load->view('include/sidebar');
			$this->load->view('include/topbar');
			$id = $this->session->userdata('userid');
	...
	function subcat_alreadyexist($key)
	{
			...
			public function delete_assetsubcat(){
			  ...
			  public function get_oneassetsubcat(){
				...
				$subpic = $this->Assetmodel->getsubpictures($id);
				$waranty = $this->Assetmodel->getwarrant_basedonsubcat($id);
				// echo $result->Measurement;
				// echo $mess = (int)$result->Measurement;
				// die();
				
				$measurement_list = $this->Assetmodel->getmeasuremnt_basedonsubcat($result->Measurement ?? 0);
			...
			public function assetpic_update(){
								...
								//if($type!='' && $cat!=''){
									...
									$where = array(
										...
										if($res == 1){
											...
											$insert = $this->Commonmodel->common_insert('AssetCatMST',$data);
												if($insert){
														echo json_encode(array('status' => 1));
													}
												else{
				...
				//////////sub cat template download start////////////
	...
	//////////sub cat template download end////////////

	//////////sub cat import start///////////////////
	...
	public function getonewarrantytype(){
	  ...
	  public function get_location_bycompanyid(){
		...
		public function asset_templatedownload(){
			...
			header('Content-Type: application/vnd.ms-excel');
			header('Content-Disposition: attachment;filename="assettype_list.xlsx"');
			$spreadsheet = new Spreadsheet();
			$sheet = $spreadsheet->getActiveSheet();
			$sheet->setCellValue('A1', 'Asset Type');
		...
		//////////////////////////end expot template
		 ...
		 /////////////////assettype import/////////////////
		 public function assettype_import(){
			...
			if($sheetcount>1)
			{
				...
				for ($i=1; $i < $sheetcount; $i++) { 
					...
					$assettypename = $sheetdata[$i][0];
				
					$where = array('AsseTypeName'=>$assettypename,'ParentID'=>$parent_id);
				
					$response = $this->Commonmodel->allreadycheck('AssettypeMst',$where);
				
					if($response==1){
					
					$data['AsseTypeName']  = $assettypename;
				 ...
				 $insert=$this->Commonmodel->common_insert('AssettypeMST',$data);
							if($insert){
								echo json_encode(array('status' => 1));
							}
							else{
