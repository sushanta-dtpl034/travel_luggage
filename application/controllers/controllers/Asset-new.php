		...
		public function getoneasset_detail_post(){
			...
			if (!empty($headers['Token']) && !empty($body) && !empty($body['assetid'])) {
				try {
						$arrdata=$this->tokenHandler->DecodeToken($headers['Token']);
						$id=$body['assetid'];
						// 	$result['assetdetail'] = $this->Assetmodel->getoneasset($assetid);
		...
		public function addasset_post(){
			...
			if (!empty($headers['Token'])) {
				try {
						...
						$warrantly_covered_for  = implode(',', $this->input->post('warrantly_covered_for'));
			...
			if (!empty($headers['Token'])) {
				try {
						...
						// $parentid = $userid;
							...
							$result['reminders'] = $this->Assetmodel->getassetlist_notify($from,$to,$parent_id);
						    $result['message'] = "success";
						    $result['status']=true;
						    return $this->set_response($result, REST_Controller::HTTP_OK);
					} catch (\Exception $e) 
		...
		public function updateassets_post(){
			...
			if (!empty($headers['Token'])) {
				try {
					...
					$delete_files =implode(',',$this->input->post('delete_files'));
					$up_warrantly_covered_for  = implode(', ',$this->input->post('up_warrantly_covered_for'));
...
public function verifyasset_post(){
			...
			if (!empty($headers['Token']) && !empty($this->input->post('view_assetid'))) {
				try {
						...
						// $createdDate = date_create($this->input->post('createdDate'));
						$location = $this->input->post('location');
						$VerificationDate = date_format(date_create($this->input->post('VerificationDate')),"Y-m-d");
	...
	$headers = apache_request_headers();
		if (!empty($headers['Token']) && !empty($body['url'])) {
			try {
					...
					$refno=openssl_decrypt ($encoded, $ciphering,$decryption_key, $options, $decryption_iv);
						
					
					$result['assetid'] = $this->Assetmodel->getid_from_qrdata($refno)->Assetid;
					$result['message'] = "success";
					$result['status']=true;
					return $this->set_response($result, REST_Controller::HTTP_OK);
				} catch (\Exception $e) 
