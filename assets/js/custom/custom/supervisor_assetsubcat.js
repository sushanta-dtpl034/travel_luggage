$( function() {
	...
	var service = $('#supervisor_assetsubcat_table').DataTable( {
		"processing": true,
		"serverSide": false,
		"order": [[ 0, "asc" ]],
		...
		"columns": [
           ...
           { "render": function ( AutoID, type, row, meta ) {
            ...
            // return '<button class="btn update_assetsub bg-success" id="'+row.AutoID+'"  datatype="edit"><i class="si si-pencil"></i></button><button class="btn ripple delete_assetsub btn-danger" id="'+row.AutoID+'"><i class="fe fe-trash"></i></button>';
          }
        }
...
//                     var datas = response.data.Measurement_Sub;
//                     arr = datas.split(",");
//                     $('#up_measurement').val(arr).change();
